        ...
        <div className="grid md:grid-cols-2 gap-6">
          {filteredMedia.map((item, i) => (
            <motion.div
              ...
              whileHover={{ y: -6, transition: { duration: 0.3 } }}
              onClick={() => handleCardClick(item.url)}
              className="group flex flex-col sm:flex-row gap-0 sm:gap-4 bg-card rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 border border-border hover:border-gold/30 cursor-pointer"
            >
              <div className="relative w-full h-48 sm:w-40 sm:h-auto flex-shrink-0 overflow-hidden">
                <img
                  src={item.img}
                  alt={item.title}
                  className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110"
              ...
              <div className="p-4 sm:p-5 flex flex-col justify-center flex-1 min-w-0">
                ...
                <p className="text-muted-foreground text-sm mt-1 line-clamp-2 sm:line-clamp-none">{item.platform}</p>
                <div className="flex items-center gap-1 mt-2 text-xs text-gold/70 group-hover:text-gold transition-colors">
                  <ExternalLink size={12} />
                  <span>Watch </span>
                </div>
